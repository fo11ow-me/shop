...
./mvnw spring-boot:run -Dspring-boot.run.profiles=dev \
  -Dspring-boot.run.jvmArguments="-Dspring.amqp.deserialization.trust.all=true"
...
E2E 测试文件位于 `mall-admin/e2e/` 和 `mall-portal/e2e/`（本地开发专用，已在 `.gitignore` 中排除）

## 关键路径

### 后端 (mall-server/)
- 主配置: `mall-server/src/main/resources/application.yml`
- 开发配置: `mall-server/src/main/resources/application-dev.yml`（MySQL + Redis localhost，本地硬编码凭据）
...
### 管理后台 (mall-admin/)
- Vite 配置: `vite.config.mjs`（端口 3002, 代理 `/admin-api` → `/admin`, UnoCSS + SCSS）
...
bash deploy/deploy.sh --skip-tests    # 跳过测试（紧急热修复）
```
...
- 管理端代理: Vite 将 `/admin-api` 代理至 `http://localhost:8800`，路径重写为 `/admin`
...
| 文件 | 策略 | 内容 |
|------|------|------|
| `application.yml` | 全环境变量占位符 `${VAR}`，无默认值 | 通用配置 + 占位符 |
| `application-dev.yml` | 本地开发硬编码值 | MySQL/Redis/RabbitMQ/JWT/OSS 凭据 |
| `application-prod.yml` | `CHANGE_ME` 占位符，通过 `.env` 注入 | 生产配置 |

- 生产环境变量由服务器 `/opt/app/mall/.env` 提供，`docker-compose.server.yml` 中引用 `${VAR}` 无默认值
- OSS 图片服务：后端先查本地缓存 `file/product/`，缓存未命中时从 OSS 下载（缓存写入失败不影响返回）

## 秒杀功能

- 秒杀场次管理: 后台 `/admin/seckill`（CRUD + 缓存同步）
- 秒杀执行: Redis Lua 脚本原子扣库存 → RabbitMQ 异步创建订单
- MQ 可靠性: 死信队列 `seckill.dlx.queue` + 3 次重试，失败回滚 Redis 库存
- 关键 Redis Key: `seckill:stock:{id}`, `seckill:order:{id}:{uid}`, `cache:seckill:active`

> ⚠️ `RedisTemplate` 序列化会给值加 JSON 双引号，Lua 脚本中需 `gsub('"','')` 去引号后再 `tonumber()`。SET 用 Lua 原生命令避免序列化污染。
